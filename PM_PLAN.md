...
**Now (2026-09-15):** **13.2.5** `[~]` **`2.13.2.5.22.12`** on **`feature/13.2.5-multi-pickup-desk`** (not `main`; **keep** the branch). Engineer 10-row SL-55 list + numbered pins **1+4** (same TT-lead frog) and **6** (dest-side after B1S) cab PASS. Dark desk + Hide strip + Cruise off + AR off + desk Ctrl+Right still PASS. **`chore/tier1-test-hardening`** parks Core `dotnet test` GitHub Action + pin-moment harvest (Observe `PinIdForStep`; dumps SU-34 / FH-82 D5I / SL-52 C1O). Four Gemini pose walks are Skipped. Cab leftover-8 / missing `#1` still open. Do not mark **13.2.5** `[x]`. Keep `feature/13.2.4.5-yard-taper`. Do not pop `stash@{0}` (5.23 kiss/Rear). Do not merge `bug/13.2.5-pin-board` wholesale.
...
| **3** | **13.2.5–13.2.6** + **13.3** `[~]` | **13.2.5** **`2.13.2.5.22.12`**. Engineer 10-row + pins 1+4 and 6 PASS. Then W5 ShowPin / remaining walk / **13.2.6** + **13.3**. |
...
- [ ] **Epic 13 — Autonomous yard / Prep loop (single job)** — **Phase C.** After **8.7** + **9.1**. Switch List **steps 1–5** (yard through Prep approach). Ships as **2.13.x**. Haul (step 6) + Delivery (step 7) live on **Epic 15**. **Simulator:** State machine on Topology + Physics; each story has a named Core gate before cab smoke.
  ...
  - [ ] **13.2 Yard prep — stack job cars** — **Split into sub-stories** (each = own ship + smoke). Parent absorbs deferred **8.10**. Full stack = **13.2.1** … **13.2.6** PASS.
    ...
    - [~] **13.2.5 Between-car / multi-pickup** — Desk **`2.13.2.5`** list PASS. Park **`2.13.2.5.22.12`**: engineer 10-row SL-55 + numbered pins **1+4** (same TT-lead) and **6** (dest-side after B1S) cab PASS. Pin-moment harvest on **`chore/tier1-test-hardening`**: Observe `PinIdForStep`; corridor dumps SU-34 / FH-82 GF-D5I / SL-52 C1O; four Gemini pose walks Skipped (dump/geometry). Cab leftover-8 CLEARED / missing `#1` still open. Dark desk + Hide strip + Cruise off (`2.13.2.5.22.7`); nearest-loco + purple job-car AR **off** (`2.13.2.5.22.5`); Maps desk **Ctrl+Right** (`2.13.2.5.22.4`). Dest itinerary **W1–W3** Gemini PASS. **Not** story `[x]`. Mini-wins: **`.1`** facing **[x]**; consist-tail CLEARED kiss **[x]** at 5.18; dest+walk **W1–W3** `[x]`; desk chord **`.22.4`** `[x]`; AR off **`.22.5`** `[x]`; desk chrome + Hide strip **`.22.6`** `[x]`; cruise-off auto-GO **`.22.7`** `[x]`; engineer pin board **`.22.12`** `[x]`; **W5** ShowPin `[~]`; **`.2`** 7.4 TryCouple **Prep only**; **`.3`** consist ⊄ job → Stop GO. Spike **`bug/13.2.5-pin-board`** stays parked (do not merge wholesale). Do not start Epic 15.
