...
- [~] **Epic 16 — Spatial Routing Infrastructure** — **Immediate priority.** Upgrade PathPlan from abstract text routing to **Spatial A\*** by feeding harvested XZ coordinates into the pathfinder. Eliminates "wrong yard" routing loopholes that forced 59 manual cab-debug patches on 13.2.5.22.\*. Ships as **2.16.x**. Exit: `dotnet test` green on spatial walks; cab confirms no manual direction correction needed.
  ...
  - [ ] **16.2 A\* heuristic** — Change PathPlan.Find from Dijkstra to A\* with Euclidean distance penalty. Log `T2 path-eval: from→to | cost=… spatial_penalty=… | REJECTED/PREFERRED`. Tier 1: A\* prefers shorter physical routes. **WIP `2.16.27`:** per-meter away-penalty stays out. First Prep couple cab **PASS** (`stop-couple` then `couple-hold`, throttle stayed 0). Cab **2.16.25** step 6 **FAIL**: kiss at `rem=2` from 27 km/h, still At switch, no CLEARED (`Smoke_step6_at_switch_after_kiss_creeps_3_until_cleared`). Desk chord missed `Input.GetKeyDown`; GUI key event is the second path (`Smoke_maps_desk_toggles_on_ctrl_insert_or_ctrl_arrows`). Neither **2.16.27** behavior has a cab PASS. C-ladder **path** lock is green (`Smoke_sl55_c4s_to_b4l_occupy_bypass_must_not_short_circuit_to_c_ladder_frog`). Cost band is green (`Smoke_cost_band_picks_closer_frog_inside_seven_seconds_and_rejects_4964_tour`). Pin 8 own-walk is green (`Smoke_pin8_loader_does_not_copy_turntable_frog`). Uncapped per-meter penalty stays out. Lock list: TEST_PLAN **Locks**. Walk: `docs/13.2.5-WALK.md` → *Frog choice*.
