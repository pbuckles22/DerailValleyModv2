...
- **8.2 `#Y` turntable origin → cross-city NoPath** — Smoke `2.8.2`: origin `#Y-#S1774#T` at SW TT → SM tracks logs `T2 route: no path`; same-yard SW→SW works. Needs yard-alias / TT pivot graph (**8.4** Town TT dest, **8.5** multi-step). Not an **8.2.1** gate unless product asks.
