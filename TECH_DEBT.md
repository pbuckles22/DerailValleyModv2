...
- **9.1.3 closed on `main` (`2.9.1.39`)** — Win 7 cab FAIL superseded by Core walker + span smoke. Ladder frozen: [docs/9.1.2_Path_Limit_Learnings.md](docs/9.1.2_Path_Limit_Learnings.md). Touch only if path/Limit regresses.
...
- **13.1.15 dest-yard-behind false positive** — `RouteHarvestTelemetry.IsDestYardBehind("SW", null, destTrackBehind: true)` is **true** (empty consist yard). Harvest line can fire when the consist city is unknown. Not a HUD bug. Revisit if **13.6.1** / dest remaining cab debug needs a clean behind edge.
