...
- **`#Y` turntable origin → cross-city NoPath** — Smoke `2.8.2`: origin `#Y-#S1774#T` at SW TT → SM tracks logs `T2 route: no path`. **8.4** shipped same-yard Set dest Turntable (`2.8.4`). Cross-city / pivot multi-leg stays **8.5**.
