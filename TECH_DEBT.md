...
- **4.2 A\* probe is not a player route** — `PathGraphSearch` is Dijkstra (h=0) O(n²) over ~2k nodes; mapper A\*s first/last `GetInstanceID()` so smoke logged `hops=—`. 117 ms `gc0=+1` at ready (H45). Revisit when **10.1** / **8.2** has a real origin→dest; do not treat `hops` as Align Route.
