...
| **Version** | **`2.9.1.14`** on `main`; **`2.9.1.20`** on **`feature/9.1.2-win1-corridor-12m`** (9.1.2 Wins 0–6). |
...
| | |
|--|--|
| **Story** | **9.1.2** Path Limit look-ahead — Wins **0–6** `[x]`; Win **7** `[ ]` |
| **Version** | **`2.9.1.20`** (feature); **`2.9.1.14`** on `main` |
| **On** | **`origin/feature/9.1.2-win1-corridor-12m` @ `bdf28bb`** (not merged) · `origin/main` @ `e219c46` |
| **Do not** | re-gather harvest; re-prove Wins 0–6; cab-debug Core; merge without **CMPH** |
| **Next** | **Win 7** Unity wire + pin smoke (`2.9.1.21`) when user says **go** |
| **Handoff** | [docs/handoff/0027-HANDOFF-2026-08-31_1818.md](docs/handoff/0027-HANDOFF-2026-08-31_1818.md) |
...
- **9.1.2 Path Limit look-ahead** — small-wins rebuild on **`feature/9.1.2-win1-corridor-12m`**. Wins **0–6** landed (`2.9.1.15`–`.20`); **Win 7** next. Ladder: [docs/9.1.2_Path_Limit_Learnings.md](docs/9.1.2_Path_Limit_Learnings.md). Fixture: `YardMasterSuite.Tests/Fixtures/Htp/boards-sw-2026-08-31.txt`. **No cab smoke until Win 7.**
...
Critical path: finish **9.1.2** (Win 7 pin smoke) → **Epic 13** → (**9.2** if needed) → **Epic 10**. Reverse-cruise gold remains cab **`feature=0`** with desk closed.

### Next

1. **9.1.2 Win 7** — Unity wire Evaluate on Maps dest; pin smoke `take 40` then `take 60` (`2.9.1.21`); same feature branch.
2. **CMPH** to `main` only when user grants — not after each win.
3. Do **not** re-prove Wins 0–6 or re-gather harvest for Win 7.
