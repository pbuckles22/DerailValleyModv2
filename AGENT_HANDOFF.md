...
| **Version** | **`2.13.2.5.22.20`** Mods (after-Prep 6/8 on 1+4 side; rest CLEARED waits At-switch; list Set Reverse travel latch; engineer 10-row + 1+4 still PASS; **`.22.20` CLEARED+Next cab due**; **not** on `main`). Stash 5.23 kiss/Rear pile stays parked. |
...
| | |
|--|--|
| **Story** | **13.2.5** `[~]` **`.22.20`** list reverse latch + after-Prep 6/8 side; **`.22.12`** 1+4; W1–W3 / desk chrome still parked (not story `[x]`); **13.2.4** `[x]` on `main` |
| **Version** | **`2.13.2.5.22.20`** |
| **On** | `origin/feature/13.2.5-multi-pickup-desk` (not merged). |
| **Do not** | merge `main`; pop `stash@{0}`; start Epic 15; re-prove 10-row list, 1+4 pins, 6/8 **side** layout, rest-not-CLEARED-until-At-switch Core, or dump-ID goldens; merge `bug/13.2.5-pin-board` wholesale; treat Skipped pose walks as leftover-8 |
| **Next** | Cab **`.22.20`**: Reverse through pin 1 → CLEARED → Next / **Ctrl+PageDown**. Then remaining **13.2.5** walk. [docs/13.2.5-WALK.md](docs/13.2.5-WALK.md). |
...
1. Park **`2.13.2.5.22.20`** is on the **product feature branch** (not `main`). Do **not** re-prove 10-row list, pins **1+4**, 6/8 **side** layout, or rest-CLEARED Session gate in Core.
2. Do **not** pop **`stash@{0}`**, merge `main`, start **Epic 15**, or merge `bug/13.2.5-pin-board` wholesale.
3. Next: cab **`.22.20`** Reverse through frog 1 until CLEARED, then Next / **Ctrl+PageDown**. Then remaining **13.2.5** walk. Rear stays 6.18 knuckle.
