...
| **[ ] 3.2** AR overlay | Unity `Camera.WorldToScreenPoint` / world-space quads. **Prefer a pre-sized marker array** (hide = move off-screen, never `Instantiate`/`Destroy` per marker). `UnityEngine.Pool.ObjectPool<T>` is optional inspect only — it can still grow/alloc; do not lock 3.2 to that type. v1 `ArMarkerProjection`, `ArMarkerTransition`, `JobCarMarkerDisplay`. Pub/Sub: radar-style time-slice in [Unity_PubSub_Best_Practices.md](Unity_PubSub_Best_Practices.md). | **adapt** fixed buffer + v1 projection math | Hitch probe fails a fixed buffer **and** DV Unity has `ObjectPool<T>` without extra allocs |
...
| Story | Leverage | Decision | Invent only if |
|-------|----------|----------|----------------|
| **[ ] 4.1** Type B mailbox | BCL `ConcurrentQueue<T>` of readonly structs; drain on main thread → Type A. Pattern already in [Unity_PubSub_Best_Practices.md](Unity_PubSub_Best_Practices.md). No UniTask / Jobs in this story. | **adapt** (thin Core queue + tests) | A zero-alloc DV mailbox exists (none known) |
| **[ ] 4.2** Track graph builder | Graph: **A\*** (standard). Game: `RailTrack` graph. Community: [WallyCZ/DVRouteManager](https://github.com/WallyCZ/DVRouteManager) `PathFinder.cs` (A\* over `RailTrack`, turntables, yard penalty). v1 `PathGraphBuilder` + `PathGraphBuildPump` (already time-sliced). Yield: **native coroutine** first (`yield return null` every N edges). Publish via **4.1**. | **adapt** A\* + v1 pump; **read** DVRouteManager for API | Coroutine still hitch-spikes → then inspect UniTask **or** Job System (Jobs struggle if nodes are managed `RailTrack` objects) |
| **[ ] 4.3** Geometry scanner (A116) | Unity physics / track bezier. DVRouteManager uses `BezierArcApproximation` for curve lookahead. v1 `SpeedLimitGeometryZones`, `TrackPathSpan`, `TrackPathAhead`. Cache until **segment change** (standard invalidation). | **adapt** native geometry + v1 cache policy | Game exposes a cheaper curvature API we are not using |
