...
**Last updated:** 2026-09-02 (**9.1.4** `[x]` on **`main`** @ **`2.9.1.40`**; keep **`feature/9.1.4-next-chip`**. **13.6.1** cab PASS waiting CMPH. **Epic 13** still open)

## Summary

**DerailValleyModv2** — Yard Master Suite v2. **Epic 3 Display Shell (infra) closed** at **3.3.1**. **Epic 4 infra closed** at **4.3**. **Epic 6 Diagnostic HUD closed** at **6.21** (`2.6.21.6`; extra pins **6.21.7** `2.13.1.16`). **Epic 7 Governors closed** at **7.5** (`2.7.5.7`). **8.7** on **`main`** (`2.8.7.31`). **9.1** + **9.1.2** + **9.1.3** + **9.1.4** on **`main`** (`2.9.1.40`). **13.1** `[x]` 7-row reverse-to-TT + leave sawtooth (`2.13.1.20`). **13.6.1** cab PASS (`2.13.6.1`) waiting CMPH. **Next:** **13.2.1** couple auto-advance. Do **not** close Epic 13. **9.2** after **13.4**. Full v1 map: [V1_FEATURE_COVERAGE.md](V1_FEATURE_COVERAGE.md). Canonical HTP: [HTP.md](HTP.md).
...
**Next:** **13.2.1** couple auto-advance from updated `main`. **13.6.1** CMPH when asked (`feature/13.6.1-remote-take`). Do not re-smoke 9.1.4 Next-chip, inbound **990152**, 13.1 7-row list, or 9.1.3 40→60 / tunnel **30**. **9.2** if needed after **13.4** → **Epic 14** Maps desk → **Epic 10**. **Deferred:** 8.8–8.9, 11 Catalog, 12 Roadside. Reverse-cruise gold remains cab **`feature=0`** with desk closed.
