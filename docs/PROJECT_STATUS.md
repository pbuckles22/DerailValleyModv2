...
**Last updated:** 2026-09-02 (**13.6.1** `[x]` on **`main`** @ **`2.13.6.1`**; keep **`feature/13.6.1-remote-take`**. **Epic 13** still open)

## Summary

**DerailValleyModv2** — Yard Master Suite v2. **Epic 3 Display Shell (infra) closed** at **3.3.1**. **Epic 4 infra closed** at **4.3**. **Epic 6 Diagnostic HUD closed** at **6.21** (`2.6.21.6`; extra pins **6.21.7** `2.13.1.16`). **Epic 7 Governors closed** at **7.5** (`2.7.5.7`). **8.7** on **`main`** (`2.8.7.31`). **9.1** + **9.1.2** + **9.1.3** + **9.1.4** on **`main`** (`2.9.1.40`). **13.1** `[x]` 7-row reverse-to-TT + leave sawtooth (`2.13.1.20`). **13.2.1** `[x]` Prep couple auto-Next (`2.13.2.1`). **13.2.2** `[x]` Prep at-track cue (`2.13.2.2`). **13.6.1** `[x]` remote take (`2.13.6.1`). **Next:** **13.4** thin (fail-closed Transit arm). **13.2.3** HOLD. Do **not** close Epic 13. **9.2** only if flat PID fails after **13.4**. Full v1 map: [V1_FEATURE_COVERAGE.md](V1_FEATURE_COVERAGE.md). Canonical HTP: [HTP.md](HTP.md).
...
| Branch | Role |
|--------|------|
| **`main`** | Integration — **13.6.1** (`2.13.6.1`). |
...
**Next:** **13.4** thin (fail-closed Transit arm) → **13.5** → **13.6** thin → **13.2.4**. Park **13.2.3** WIP; multi-area FILO / **10.2** after walk-in. Gemini PASS 2026-09-02. Do not re-smoke 13.6.1 GO take, 13.2.2 at-track, 13.2.1 couple-next, 9.1.4 Next-chip, inbound **990152**, 13.1 7-row list, or 9.1.3 40→60 / tunnel **30**. **9.2** only if flat PID fails after **13.4**. **Deferred:** 8.8–8.9, 11 Catalog, 12 Roadside, Epic 14 until after 13. Reverse-cruise gold remains cab **`feature=0`** with desk closed.
...
| 13.4 thin | 2–3 | | | | fail-closed Transit arm |
